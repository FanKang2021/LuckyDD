...
exports.manualUpdate = exports.startAutoUpdate = exports.resetDatabase = exports.updateDatabase = exports.fetchLotteryData = exports.crawlerConfigs = void 0;
    ...
    },
    {
        ...
        parseFunction: (html) => {
            ...
            $('.kjhTable tbody tr').each((_, row) => {
                ...
                if (!isNaN(back1))
                    backNumbers.push(back1);
                if (!isNaN(back2))
                    backNumbers.push(back2);
...
const fetchLotteryData = async (config, retryCount = 3) => {
...
exports.fetchLotteryData = fetchLotteryData;
const updateDatabase = async () => {
    ...
    for (const config of exports.crawlerConfigs) {
        try {
            const newData = await (0, exports.fetchLotteryData)(config);
            if (newData.length === 0) {
                errors.push(`${config.name}: 没有获取到新数据`);
                continue;
            }
            const existingData = database[config.lotteryType] || [];
            const existingIssues = new Set(existingData.map((d) => d.id));
            const newEntries = newData.filter((d) => !existingIssues.has(d.id));
            if (newEntries.length > 0) {
                database[config.lotteryType] = [...existingData, ...newEntries].sort((a, b) => a.id.localeCompare(b.id));
...
const startAutoUpdate = (intervalHours = 12) => {
    console.log(`自动更新已启动，每 ${intervalHours} 小时更新一次数据`);
    const update = async () => {
        console.log('开始更新彩票数据...');
        const result = await (0, exports.updateDatabase)();
        if (result.success) {
